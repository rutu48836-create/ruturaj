...
import { Trash2, SendHorizontal, Search,Ellipsis,RefreshCw,Share,Plus} from "lucide-react"
...
<div className={`${styles.existing_projects_section} ${projects.length === 0 ? styles.empty_state : ''}`}>          {projects.length === 0 ? (
 ...
 { /*<img src={} alt='no project' className={styles.mobile_banner}/> */}
...
><div className={styles.project_branding} style={{
 ...
 <div className={styles.chatbot_demo_head}  style={{
...
<img src={project.logo_url} alt="no logo" style={{width:'24px',height:'24px',display:'flex',borderRadius:'50%',marginLeft:'10px'}}/>
  <small>{project.name}</small> <div className={styles.icon_demo}><RefreshCw size={12}/><Share size={12} style={{marginRight:'8px'}}/></div>
  </div>   
...
<div style={{...userMessageStyle}}>
 ...
 <div style={{ ...userMessageStyle, alignSelf: 'flex-start', marginLeft: '10px', backgroundColor:project.color, color: '#ffffff' }}>
  </div>
  ...
  <small>{project.agent_type} agent </small> 
 {/* {project.agent_type === 'assistant' && (
  <button onClick={() => {
    window.location.href = `https://ruturaj-2.onrender.com/auth/google?shareToken=${project.share_token}`
  }}> Connect
...
<button className={styles.cancel_btn1}  disabled={delete_confirm}  onClick={() => {
